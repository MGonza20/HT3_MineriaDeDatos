...
## Modelo univariable 1
Conjunto de entrenamiento (`r nrow(train)` muestras):  
```{r echo=FALSE}
head(trainingSet)
```

Conjunto de prueba (`r nrow(test)` muestras):  
```{r echo=FALSE}
head(testingSet)
```

### Estimación de precio usando el año de construcción de la casa 
yearBuilt se toma como la variable independiente y se utiliza para hacer el modelo univariado de regresión lineal para predecir el precio de las casas.
```{r}
singleVariableModel <- lm(salePrice~yearBuilt, data = trainingSet)
```

Caracteristicas del modelo univariado entrenado:
```{r}
summary(singleVariableModel)
```
Los coeficientes en este modelo son significativos. Además podemos saber en base a estos datos que hay una relación significativa entre salePrice y yearBuilt en el conjunto de datos de entrenamiento. Sin embargo $R^2$ es considerado bajo, por tanto el modelo logra explicar una pequeña cantidad de la variación en salePrice.


Ecuación de regresión:
$salePrice = `r round(singleVariableModel$coefficients[2],2)`yearBuilt + `r round(singleVariableModel$coefficients[1],2)`$  

```{r}
ggplot(data = trainingSet, mapping = aes(x = yearBuilt, y = salePrice)) +
geom_point(color = "green", size = 2) +
geom_smooth(method = "lm", se = TRUE) +
labs(title = "Precio de venta vs Año de construcción", x = "Año de construcción", y = "Precio de venta") +
...
```{r}
predSP<-predict(singleVariableModel, newdata = testingSet)
```
...
```{r}
head(singleVariableModel$residuals)
```

### Gráficos para analizar residuales
```{r}
plot(singleVariableModel)
```
En el gráfico ***Residuals vs Fitted*** se puede ver que hay un leve patrón.

Chequeo de distribución aleatoria de los puntos:
```{r}
hist(singleVariableModel$residuals)
boxplot(singleVariableModel$residuals)
```
Según los gráficos no se puede establecer que haya una distribución normal. Dado que en el histograma la gráfica no tiene una distribución normal y las columnas están hacia la izquierda. Respecto al qqplot los datos están alejados del centro.


```{r}
library(nortest)
lillie.test(singleVariableModel$residuals)
```

### Hallando el precio de venta para el conjunto de prueba  
```{r}
predPS <-predict(singleVariableModel, newdata = testingSet[,c(1,2,4)])
library(caret)
RMSE(singleVariableModel,testingSet$priceSale)
```

```{r}
plot(singleVariableModel$priceSale,col="blue")
points(singleVariableModel, col="red")
```

## Modelo univariable 2
...
```
Los coeficientes en este modelo son significativos. Además se puede observar que el valor de $R^2$ es de 0.52 siendo considerablemente mayor al valor de $R^2$ que se obtuvo al usar la variable independiente yearBuilt, por tanto este modelo explica mejor los datos.
...
geom_point(color = "green", size = 2) +
geom_smooth(method = "lm", se = TRUE) +
labs(title = "Precio de venta vs Área de vivienda", x = "Área de vivienda", y = "Precio de venta") +
theme_bw() + theme(plot.title = element_text(hjust = 0.5))
```
...
```
En el gráfico ***Residuals vs Fitted*** se puede ver que los datos se encuentran distribuidos de forma aleatoria alrededor de 0 y no siguen un patrón específicamente.
...
```
Según los gráficos se puede observar que en el histograma obtenido se tiene una forma relativamente normal, por otra parte en el gráfico q-q los extremos se alejan moderadamente de la línea y en la caja de bigotes se observa que hay varios datos que se encuentran en los extremos, por tanto nos indica que los datos se encuentran en un rango normal.
...
```
Haciendo una prueba de normalidad el valor de p es mayor que 0.05 por tanto no se rechaza la hipótesis nula de normalidad y se puede afirmar que los residuos siguen una distribución normal.


### Precio de venta para el conjunto de prueba
```{r}
predSPP2<-predict(singleVariableModel2, newdata = testingSet[,c(1,2,4)])
library(caret)
RMSE(predSPP2,testingSet$salePrice)
```

```{r}
plot(testingSet$priceSale,col="blue")
points(predSPP2, col="red")
```

```{r}
summary(testingSet$priceSale-predSPP2)
```
...
modelCorrelations <- cor(model_output, method="pearson")
