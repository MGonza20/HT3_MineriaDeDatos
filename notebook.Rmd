...
#### Asignacion de valores para vecindarios
...
#### Asignación de valores para estilo de casa
...
#### Asignación de valores para materiales de casa
```{r}
f <- c("PConc", "CBlock", "BrkTil", "Slab", "Stone")

for(n in 1:length(f)) {
  # Variable minuscula para nuestro uso.
  data$foundation[data$Foundation == f[n]] <- n
}
data$Foundation <- NULL
```

#### Asignación de valores para condicion de venta
```{r}
sc <- c("Abnorml", "AdjLand", "Alloca", "Family", "Normal", "Partial")

for(n in 1:length(sc)) {
  # Variable minuscula para nuestro uso.
  data$saleCondition[data$SaleCondition == sc[n]] <- n
}
data$SaleCondition <- NULL
```

#### Asignación de valores para tipo de venta
```{r}
...
```{r}
dataH <- subset(data, select = -c(Alley, LotShape, LotConfig, LandSlope, Fence, MiscFeature, MiscVal, ScreenPorch, EnclosedPorch, Functional, BldgType))
```
