...
### Propuesta 2
```{r}
grouped_data <- data %>% group_by(MSZoning, YearBuilt)
agg_tbl <- grouped_data %>% summarise(mean(SalePrice))
grouped_dataset <- as.data.frame(agg_tbl)
colnames(grouped_dataset)[colnames(grouped_dataset) == "MSZoning"] = "Zona"
colnames(grouped_dataset)[colnames(grouped_dataset) == "YearBuilt"] = "Año de Construcción"
colnames(grouped_dataset)[colnames(grouped_dataset) == "mean(SalePrice)"] = "Promedio de Precio (USD)"
grouped_dataset <- grouped_dataset[order(grouped_dataset$`Zona`, grouped_dataset$`Año de Construcción`, decreasing = FALSE), ]
kable(grouped_dataset, caption = "Promedio de ventas por cada zona y año de construcción")
```
...
colnames(grouped_dataset)[colnames(grouped_dataset) == "mean(SalePrice)"] ="Promedio de Precio (USD)"
grouped_dataset <- grouped_dataset[order(grouped_dataset$Vecindario, decreasing = FALSE), ]
kable(grouped_dataset, caption = "Promedio de ventas por cada vecindario")
```

```{r}
ggplot(data, aes(x = SalePrice)) + geom_histogram(binwidth = 25000, color = "green") +
ggtitle("Distribución del precio de venta") + xlab("Precio de venta (USD)") + ylab("Conteo")
