...
Conjunto de entrenamiento (`r nrow(trainingSet)` muestras):  
```{r echo=FALSE}
head(trainingSet)
```

Conjunto de prueba (`r nrow(testingSet)` muestras):  
```{r echo=FALSE}
head(testingSet)
```
...
```{r}
predPS <-predict(singleVariableModel, newdata = testingSet[,])
library(caret)
```

```{rplot(singleVariableModel$priceSale,col="blue")
...
```{r}
predSPP2<-predict(singleVariableModel2, newdata = testingSet[,])
library(caret)
